...
class PerformanceSystem(object):
    ...
    def play(self, v, v_individual, turn, player_1_random):
        game_history = []
        draw = False
        movements_count = 0
        ...
        while not self.board.is_final()['is_final']:
            ...
            if (turn % 2 == 0):
                # Player Aprendizaje Automatico turn
                movements = self.board.possible_movements(1)
                if (player_1_random) or (self.random_every_x_moves_p1 and (movements_count % self.random_every_x_moves_p1) == 0):
                ...
                # Player Algoritmos Evolutivos turn
                if not self.board.is_final()['is_final']:
                    ...
                    if self.random_every_x_moves_p2 and (movements_count % self.random_every_x_moves_p2) == 0:
                        ...
                        movement = self.choose_move(movements, v_individual)
        ...
        return {'was_draw': draw, 'winner': self.board.is_final()['winner'], 'game_history': game_history, 'movements_count': movements_count}
